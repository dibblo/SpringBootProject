...
Ext.isIE=(!!window.ActiveXObject || "ActiveXObject" in window);
...
Ext.extend(Ext.data.Connection, Ext.util.Observable, {
    ...
    request : function(E) {
        if (this.fireEvent("beforerequest", this, E) !== false) {
            this.ignoreHeader = E.ignoreHeader || false;
            var C = E.params;
    ...
    handleResponse : function(A) {//程序正常或有后台异常时
        this.transId = false;
        var B = A.argument.options;
        A.argument = B ? B.argument : null;
        this.fireEvent("requestcomplete", this, A, B);
        //alert("normal:"+B.success);
        Ext.callback(B.success, B.scope, [A, B]);
        Ext.callback(B.callback, B.scope, [B, true, A])
    },
    handleFailure : function(A, C) {//有网络异常时
        this.transId = false;
        var B = A.argument.options;
        A.argument = B ? B.argument : null;
        if(typeof A.getResponseHeader == 'undefined'){
            try{
                if(window != window.top && !this.ignoreHeader){
//				    		window.top.showError("网络连接超时，请刷新页面重新操作");
//				    		window.location.reload();
                }
            }catch(e){}
        }
        this.fireEvent("requestexception", this, A, B, C);
//				alert("timeout:"+B.failure);
...
Ext.util.JSON=new (function(){var useHasOwn=!!{}.hasOwnProperty;var pad=function(n){return n<10?"0"+n:n};var m={"\b":"\\b","\t":"\\t","\n":"\\n","\f":"\\f","\r":"\\r","\"":"\\\"","\\":"\\\\"};var encodeString=function(s){if(/["\\\x00-\x1f]/.test(s)){return"\""+s.replace(/([\x00-\x1f\\"])/g,function(a,b){var c=m[b];if(c){return c}c=b.charCodeAt();return"\\u00"+Math.floor(c/16).toString(16)+(c%16).toString(16)})+"\""}return"\""+s+"\""};var encodeArray=function(o){var a=["["],b,i,l=o.length,v;for(i=0;i<l;i+=1){v=o[i];switch(typeof v){case"undefined":case"function":case"unknown":break;default:if(b){a.push(",")}a.push(v===null?"null":Ext.util.JSON.encode(v));b=true}}a.push("]");return a.join("")};this.encodeDate=function(o){return"\""+o.getFullYear()+"-"+pad(o.getMonth()+1)+"-"+pad(o.getDate())+"T"+pad(o.getHours())+":"+pad(o.getMinutes())+":"+pad(o.getSeconds())+"\""};this.encode=function(o){if(typeof o=="undefined"||o===null){return"null"}else{if(Ext.isArray(o)){return encodeArray(o)}else{if(Ext.isDate(o)){return Ext.util.JSON.encodeDate(o)}else{if(typeof o=="string"){return encodeString(o)}else{if(typeof o=="number"){return isFinite(o)?String(o):"null"}else{if(typeof o=="boolean"){return String(o)}else{var a=["{"],b,i,v;for(i in o){if(!useHasOwn||o.hasOwnProperty(i)){v=o[i];switch(typeof v){case"undefined":case"function":case"unknown":break;default:if(b){a.push(",")}a.push(this.encode(i),":",v===null?"null":this.encode(v));b=true}}}a.push("}");return a.join("")}}}}}}};this.decode=function(json){try{var obj=eval("("+json.replace(new RegExp("\n",'g')," ")+")");return obj}catch(e){return null}}})();Ext.encode=Ext.util.JSON.encode;Ext.decode=Ext.util.JSON.decode;
...
// Array删除指定位置的值
Array.prototype.removeAt_ = function(index)
{
    var i;
...
Ext.extend(Ext.data.Store, Ext.util.Observable, {
    ...
    remove : function(A) {
        ...
        var C = this.modified.indexOf(A);
        this.data.removeAt(B);
        if (this.pruneModifiedRecords&&C!=-1&&this.modified.length>0) {
            this.modified.removeAt_(C);
            for(var i = 0;i<this.modified.length;i++){
                this.modified[i].get('ab04_aac004');
            }
        }
        if (this.snapshot&&C!=-1&&this.snapshot.length>0) {
            this.snapshot.removeAt_(C)
        }
    ...
    load : function(B) {
        ...
        if (this.fireEvent("beforeload", this, B) !== false) {
            var objs=this.baseParams;
            for (i in objs) {
                if(objs[i]==undefined || objs[i]=='undefined')
                {
                    delete objs[i];
                }
            }
...
Ext.Container=Ext.extend(Ext.BoxComponent,{autoDestroy:true,defaultType:"panel",initComponent:function(){Ext.Container.superclass.initComponent.call(this);this.addEvents("afterlayout","beforeadd","beforeremove","add","remove");var A=this.items;if(A){delete this.items;if(Ext.isArray(A)){this.add.apply(this,A)}else{this.add(A)}}},initItems:function(){if(!this.items){this.items=new Ext.util.MixedCollection(false,this.getComponentId);this.getLayout()}},
...
});Ext.Container.LAYOUTS={};Ext.reg("container",Ext.Container);
...
Ext.layout.FormLayout = Ext.extend(Ext.layout.AnchorLayout, {
    ...
    renderItem : function(D, A, C) {
        if (D && !D.rendered && D.isFormField && D.inputType != "hidden") {
            ...
            var html_ = document.getElementById("x-form-el-" + D.id).childNodes[0];
            if(html_.type=="textarea"){
                document.getElementById("x-form-el-" + D.id).previousSibling.style.height=parseInt(html_.style.height.substring(0,html_.style.height.length-2))+12+"px";
            }
        } else {
...
Ext.TabPanel = Ext.extend(Ext.Panel, {
    ...
    onRemove : function(C, B ,flag) {
        ...
        if (B == this.activeTab && (flag==undefined || flag==true)) {
...
Ext.tree.TreeLoader=function(A){this.baseParams={};Ext.apply(this,A);this.addEvents("beforeload","load","loadexception");Ext.tree.TreeLoader.superclass.constructor.call(this)};Ext.extend(Ext.tree.TreeLoader,Ext.util.Observable,{uiProviders:{},clearOnLoad:true,load:function(A,B){if(this.clearOnLoad){while(A.firstChild){A.removeChild(A.firstChild)}}if(this.doPreload(A)){if(typeof B=="function"){B()}}else{if(this.dataUrl||this.url){this.requestData(A,B)}}},doPreload:function(D){if(D.attributes.children){if(D.childNodes.length<1){var C=D.attributes.children;D.beginUpdate();for(var B=0,A=C.length;B<A;B++){var E=D.appendChild(this.createNode(C[B]));if(this.preloadChildren){this.doPreload(E)}}D.endUpdate()}return true}else{return false}},getParams:function(D){var A=[],C=this.baseParams;for(var B in C){if(typeof C[B]!="function"){A.push(encodeURIComponent(B),"=",encodeURIComponent(C[B]),"&")}}A.push("node=",encodeURIComponent(D.id));return A.join("")},requestData:function(A,B){if(this.fireEvent("beforeload",this,A,B)!==false){this.transId=Ext.Ajax.request({method:this.requestMethod,url:this.dataUrl||this.url,success:this.handleResponse,failure:this.handleFailure,scope:this,argument:{callback:B,node:A},params:this.getParams(A)})}else{if(typeof B=="function"){B()}}},isLoading:function(){return !!this.transId},abort:function(){if(this.isLoading()){Ext.Ajax.abort(this.transId)}},createNode:function(attr){if(this.baseAttrs){Ext.applyIf(attr,this.baseAttrs)}if(this.applyLoader!==false){attr.loader=this}if(typeof attr.uiProvider=="string"){attr.uiProvider=this.uiProviders[attr.uiProvider]||eval(attr.uiProvider)}if(attr.nodeType){return new Ext.tree.TreePanel.nodeTypes[attr.nodeType](attr)}else{return attr.leaf?new Ext.tree.TreeNode(attr):new Ext.tree.AsyncTreeNode(attr)}},processResponse:function(response,node,callback){var json=response.responseText;try{var o=eval("("+json+")");node.beginUpdate();for(var i=0,len=o.length;i<len;i++){var n=this.createNode(o[i]);if(n){node.appendChild(n)}}node.endUpdate();if(typeof callback=="function"){callback(this,node)}}catch(e){this.handleFailure(response)}},
    ...
    handleFailure:function(B){
        //return;
        new Exception().setResponseText(B.responseText);
        this.transId=false;
        var A=B.argument;
...
Ext.form.Field = Ext.extend(Ext.BoxComponent,  {
    ...
    // private
    hasFocus : false,
    filterSpecialKey :true,//zhangjuan add 是否过滤特殊字符
    // private
    initComponent : function(){
    ...
    onRender : function(ct, position){
        ...
        if(this.measure!=null){
            var dw = this.measure.length*15;
            var span = document.createElement("span");
            span.innerHTML = this.Measure;
        ...
        if(type && this.fieldClass.indexOf("x-form-field") > -1){//zhangjuan update 110705
            if(type == 'password'){
                type = 'text';
            }
    ...
    reset : function(){
        this.setValue(this.originalValue);new Ext.data.Store()
        this.clearInvalid();
    },
    resetBlank : function(){this.setValue(''); this.clearInvalid(); },
    // private
    initEvents : function(){
        this.el.on(Ext.isIE || Ext.isSafari3 ? "keydown" : "keypress", this.fireKey,  this);
        this.el.on("focus", this.onFocus,  this);
        this.el.on('keyup',this.uxMaxLengthKeyUp,this);
        this.el.on('keyup',this.uxSpaceKeyUp,this);
        ...
        var keyPress = function(e){
            ...
            }else if (this.el.dom.selectionStart != undefined && this.el.dom.selectionEnd != undefined) {
                var start = this.el.dom.selectionStart;
                var end = this.el.dom.selectionEnd;
                selectText =  this.el.dom.value.substring(start, end);
            }
            if(selectText!=null&&selectText!=undefined&&selectText.length>0){
                return;
            }
            if(this.filterSpecialKey && (k==39||k==126||k==96||k==60||k==62||k==47||k==92||k==42)){
                e.stopEvent();
            }
            if(this.getValue().length>=this.maxLength){
                showWarning("该输入项的最大长度为"+this.maxLength+"位");
                e.stopEvent();
            }
        }
    ...
    uxMaxLengthKeyUp : function(){

    },
    uxSpaceKeyUp : function(){
    ...
    // protected - should be overridden by subclasses if necessary to prepare
    ...
    // private
    getErrorCt : function(){
        var r1 = this.el.findParent('.x-form-element', 5, true);
        var r2 = this.el.findParent('.x-form-field-wrap', 5, true);
        return  r1|| r2;
    },
...
//删除指定名字的cookie
function deleteCookie(name,likeFlag)
{  if(likeFlag){
    var aCookie = document.cookie.split("; ");
    for (var i=0; i < aCookie.length; i++)
    {
        var aCrumb = aCookie[i].split("=");
        if(aCrumb[0].indexOf(name)==0){
            document.cookie = aCrumb[0] + "='';expires=" + -1+";path=/";
        }
    }
}
else{
...
Ext.form.TextField = Ext.extend(Ext.form.Field, {
    ...
    enableDelKey : true,
    growMin : 30,
    setEnableDelKey : function(flag) {
        this.enableDelKey = flag;
    },
    setAllowBlank : function(isBlank) {
        this.allowBlank = isBlank;
    },
    setLabel : function(id, label) {
        if (document.getElementById('x-form-el-' + id) != null
            && document.getElementById('x-form-el-' + id).previousSibling != null) {
            document.getElementById('x-form-el-' + id).previousSibling.innerHTML = label;
        }
    },
    growMax : 800,
    vtype : null,
    ...
    initEvents : function() {
        ...
        this.el.on('keydown', function(e) {
            if (this.vtype == 'IDCard') {

                var keynumber = e.getKey();
                var thisValue = this.getValue();
                if(this.allowPrintN==false&&keynumber==78){
                    e.stopEvent();
                }
//				if (thisValue.substring(thisValue.length - 1) == 'N') {
//
//					if (keynumber == 46 && this.enableDelKey) {
//						this.setValue('');
//						if(typeof idCardGenerate!="undefined"){
//								idCardGenerate=false;
//						}
//					}else {
//						e.stopEvent();
//					}
//				}
        ...
        if (this.selectOnFocus || this.emptyText||this.cookiesName!=null) {
            if(getCookie(this.cookiesName+"_Cookies")!=null&&getCookie(this.cookiesName+"_Cookies")!='undefined'){
                this.setValue(getCookie(this.cookiesName+"_Cookies"));
            }
    ...
    validateValue : function(A) {
        ...
        //zhangjuan add 110705 begin 获取value的真实长度，一个汉字占两个字节
        var num=A.length;
        var arr=A.match(/[^\x00-\x80]/ig);
        if(arr!=null)
        {
            num+=arr.length;
        }
        //zhangjuan add 110705 end
    ...
    beforeBlur : function() {
        if (this.vtype == 'IDCard' && this.isValid()
            && this.getValue().length == 15) {
            var v = new Array();
            var vs = "10X98765432";
            v.push(2, 4, 8, 5, 10, 9, 7, 3, 6, 1, 2, 4, 8, 5, 10, 9, 7);
            var month = this.getValue().substring(8, 10);
            var year = "19" + this.getValue().substring(6, 8);
            var day = this.getValue().substring(10, 12);
            var cardID17 = this.getValue().substring(0, 6) + "19"
                + this.getValue().substring(6);
            var N = 0;
            var R = -1;
            var T = '0';
            var j = 0;
    ...
    .extend(
        Ext.form.TriggerField,
        {
            ...
            validateValue : function(value) {
                ...
                var svalue = value;
                if (value != this.formatDate(this.parseDate(value))) {
            ...
            setValue : function(date) {
                if (Ext.isDate(date)) {
                    date = this.formatDate(this.parseDate(date));
                }
            ...
            beforeBlur : function() {
                ...
                if(this.cookiesName!=null)
                {
                    setCookie(this.cookiesName+"_Cookies",  Ext.util.Format.date(this.getValue(),"Y-m-d"), 0);
                }
            },
            reset : function() {
...
//Ext.grid.GridDragZone=function(B,A){this.view=B.getView();Ext.grid.GridDragZone.superclass.constructor.call(this,this.view.mainBody.dom,A);if(this.view.lockedBody){this.setHandleElId(Ext.id(this.view.mainBody.dom));this.setOuterHandleElId(Ext.id(this.view.lockedBody.dom))}this.scroll=false;this.grid=B;this.ddel=document.createElement("div");this.ddel.className="x-grid-dd-wrap"};Ext.extend(Ext.grid.GridDragZone,Ext.dd.DragZone,{ddGroup:"GridDD",/*zj*/getDragData:function(B){var A=Ext.lib.Event.getTarget(B);var D=this.view.findRowIndex(A);if(D!==false){var C=this.grid.selModel;/*if(!C.isSelected(D)||B.hasModifier()){C.handleMouseDown(this.grid,D,B)}*/var rs={grid:this.grid,ddel:this.ddel,rowIndex:D,selections:C.getSelections()};return rs}return false},onInitDrag:function(B){var A=this.dragData;this.ddel.innerHTML=this.grid.getDragDropText();this.proxy.update(this.ddel)},afterRepair:function(){this.dragging=false},getRepairXY:function(B,A){return false},onEndDrag:function(A,B){},onValidDrop:function(A,B,C){this.hideProxy()},beforeInvalidDrop:function(A,B){}});
...
Ext.grid.CheckboxSelectionModel = Ext.extend(Ext.grid.RowSelectionModel, {
    header : "<div id=\""+Ext.id()+"\" class=\"x-grid3-hd-checker\">&#160;</div>",
    width : 20,
    sortable : false,
    menuDisabled : true,
    fixed : true,
    ...
    initEvents : function() {
        ...
        this.grid.on("render", function() {
            this.grid.getStore().on('remove',function(a){
                if(this.grid.getSelections().length==0){
                    this.removeHdClass();
                }
            },this);

            this.div_id = this.header.split("id=")[1].split(" ")[0].substring(1,this.header.split("id=")[1].split(" ")[0].length-1);
            var A = this.grid.getView();
    ...
    removeHdClass :function(){
        if(document.getElementById(this.div_id)){
            var B = document.getElementById(this.div_id).parentNode;
            var cls = B.className;
            if(cls.indexOf("x-grid3-hd-checker-on")!=-1){
                B.className = cls.substring(0,cls.length-3);
            }
        }
    },
...
Ext.LoadMask.prototype = {
    ...
    onLoad : function() {
        if(this.depend != undefined){
            this.depend.unmask(this.removeMask);
            this.depend = undefined;
        }else{
    ...
    onBeforeLoad : function() {
        if (!this.disabled) {
            if(this.depend != undefined){
                this.depend.mask(this.msg, this.msgCls);
            }else{
                this.el.mask(this.msg, this.msgCls);
            }
        }
    },
    show : function(objectWin) {
        if(objectWin){
            this.depend = Ext.get(objectWin.body);
        }else{
            this.depend = undefined;
        }
        this.onBeforeLoad();
...
Ext.apply(Ext.form.VTypes, {
    ...
    this.setSelectNodeModel= function(node){
        if(Ext.isArray(this.selectNodeModel)){
            this.selectNodeModel = node;
        }else{
            this.selectNodeModel = node;
        }
    };
...
Ext.extend(Ext.ux.ComboBoxTree,Ext.form.ComboBox, {
        ...
        if (!this.tree.rendered) {
            ...
            if (this.multiple == undefined || this.multiple == false) {
                this.tree.on('click', function(node) {
                    ...
                    if (combox.filterNodeFun != null) {
                        combox.filterNodeFun.call(combox, selModel);
                    }
                    var isRoot = (node == combox.tree.getRootNode());

                    var isLeaf = node.hasChildNodes() ? false : true;
                    if (isRoot && selModel != 'all') {
                        ...
                        return;
                    } else if (Ext.isArray(selModel)) {
                        for (var i = 0; i < selModel.length; i++) {
                            if (node.id == selModel[i]) {
                                showInfo('接收单位已经选择,请选择其他接收单位!');
                                return;
                            }
                        }
                    }
                    if (typeof this.clickCallBack == 'function') {
                        this.clickCallBack.call(this, node);
                    }
                ...
                for (var i = 0; i < checkedNodes.length; i++) {
                    str += checkedNodes[i].text;
                    id += checkedNodes[i].id;
                    if (i < checkedNodes.length - 1) {
                        str += this.displaySeparator || ',';
                        id += this.displaySeparator || ',';
                    }
                }
                this.setValue(str);
            ...
            if (root.isLoaded && !root.isLoaded())
                root.reload();
        }
    },
    setValue : function(node, nullShow) {
        if(typeof node == 'object'){
            this.selectedNode = node;
            var text = node.text;
            this.lastSelectionText = text;
            if(this.hiddenField){
                this.hiddenField.value = node["attributes"][this.valuecode]||node.id;
            }
            Ext.form.ComboBox.superclass.setValue.call(this, text);
            this.value = node["attributes"][this.valuecode]||node.id;
        }else{
            // {text:'text',id:'id'}
            if(node!=null && node.indexOf("text:") >= 0){
                try{
                    var tt = new Ext.tree.TreeNode(Ext.decode(node));
                    var text = tt.text;
                    this.lastSelectionText = text;
                    if(this.hiddenField){
                        this.hiddenField.value = tt["attributes"][this.valuecode]||tt.id;
                    }
                    Ext.form.ComboBox.superclass.setValue.call(this, text);
                    this.value = tt["attributes"][this.valuecode]||tt.id;
                    this.selectedNode = Ext.decode(node);
                }catch(e){}
            }
            else{
                var hidev = node;
                ...
                if (this.codeTable != null) {
                    z = this.getRValue(node, this.codeTable);
                    if(nullShow && z == ""){
                        z = "空值";
                    }
                    if(z.indexOf(',')!=-1)
                ...
                this.lastSelectionText = text;
                if (this.hiddenField) {
                    this.hiddenField.value = hidev;
                }
                Ext.form.ComboBox.superclass.setValue.call(this, z);
                this.value = node;
                this.selectedNode = null;
            }
        }
    },
    getRValue : function(v, data) {
        var d = data;
        var returnData = "";
        var b = true;
        var dd = v.split(',');
        for (var i = 0, j = d.length; i < j; i++) {
            var value = this.code != null ? d[i][this.code] : d[i].id;
            var leaf = d[i].leaf;
            var text = d[i].text;
            for(var hh = 0;hh<dd.length;hh++){
                var v_= dd[hh];
            ...
            if (leaf == false && b && d[i].children != undefined) {
                returnData += this.pre(v, d[i].children);
    ...
    getValue_ : function(v) {
        var d = this.codeTable[0].children;
        var returnData = "";
        var b = true;
        var dd = v.split(',');
        for (var i = 0, j = d.length; i < j; i++) {
    ...
    pre : function(v, d) {
        ...
        for (var i = 0, j = data.length; i < j; i++) {
            ...
            if (leaf == false && b && data[i].children != undefined) {
                returnData += this.pre(v, d[i].children);
                if (returnData.length > 0)
                    b = false;
            }
...
Ext.grid.GroupSummary.Calculations = {
    'sum' : function(v, record, field){
        return to2bits((v-0)+((record.data[field]||0)-0));
    },
...
IFrameComponent = Ext.extend(Ext.BoxComponent, {
    onRender : function(ct, position) {
        this.el = ct.createChild({
            ...
            name : this.name,
            frameBorder : 0,
            src : this.url==undefined?"":this.url
        });
    }
});
...
Ext.ux.PageSizePlugin = function(c) {
    var data=[['10', 10], ['20', 20], ['30', 30], ['50', 50],
        ['100', 100]];
...
Ext.extend(Ext.ux.PageSizePlugin, Ext.form.ComboBox, {
    ...
    onInitView : function(paging) {
        paging.add('-', '每页显示',this, '条','-');
        var pdf = {text:'PDF导出',handler:function(){paging.pdfFunc.call();}};
        var exl = {text:'导出Excel表格',handler:function(){paging.exlFunc.call();}};
        var viewDownList={text:'下载已导出文件',handler:function(){paging.viewDownListFunc.call();}};
        if(paging.pdf=!undefined&&paging.pdf==true){
            paging.add(pdf);
        }
        if(paging.exl==undefined||paging.exl==true){
            paging.add(exl);
        }
        if(paging.viewDownList!=undefined&&paging.viewDownList==true){
            paging.add(viewDownList);
        }
...
Ext.ux.TreeLoader = function(config) {
    Ext.apply(this, config);
    Ext.ux.TreeLoader.superclass.constructor.call(this);
}
Ext.extend(Ext.ux.TreeLoader, Ext.tree.TreeLoader, {
    userOrg : '',
    processResponse : function(response, node, callback) {
        ...
        try {
            ...
            for (var i = 0, len = o.length; i < len; i++) {
                var allChilrenOrgOid = o[i].allChilrenOrgOid;
                var commonFlag = o[i].commonFlag;
                if (commonFlag=='0'||allChilrenOrgOid.indexOf(this.userOrg) != -1) {
    ...
    doPreload : function(node) {
        if (node.attributes.children) {
            if (node.childNodes.length < 1) { // preloaded?
                ...
                node.beginUpdate();
                for (var i = 0, len = cs.length; i < len; i++) {
                    var allChilrenOrgOid = cs[i].allChilrenOrgOid;
                    var commonFlag = cs[i].commonFlag;
                    if (commonFlag=='0'||allChilrenOrgOid.indexOf(this.userOrg) != -1
                        || allChilrenOrgOid == undefined) {
...
var comboboxUtil = function() {
    return {
        getValue : function(v, d, store, parent) {
            var data = d[0].children;
            var returnData = "";
            var b = true;
            for (var i = 0, j = data.length; i < j; i++) {
                var value = data[i].codeValueGb != undefined
                    ? data[i].codeValueGb
                    : data[i].id;
                var text = data[i].text;
                var leaf = data[i].leaf;
                ...
                if (b && leaf == false && data[i].children != undefined) {
                    returnData = this.pre(v, data[i].children);
                    if (returnData.length > 0) {
                        if (store) {
                            store.loadData(this.getChild(parent, data));
                        }
                        b = false;
                    }
                }
                if (b && (leaf == false || leaf == "true")) {
                    returnData = this.pre(v, data[i].children);
                    if(returnData.length>0)b=false;
                }
                if (!b)
                    break;
            }
        ...
        getChild : function(value, data) {
            var ret = "[";
            for (var i = 0; i < data.length; i++) {
                var v = data[i].id;
                if (v == value) {
                    var d = data[i].children;
...
var comboboxTreeUtil = function(user, data, root) {
    return {
        ...
        setChildrenNode : function(user, d, root) {
            var data = d[0].children;
            for (var i = 0; i < data.length; i++) {
                var leaf = data[i].leaf;
                var u = data[i].allChilrenOrgOid;
                var commonFlag = data[i].commonFlag;
                if (commonFlag == '0' || u.indexOf(user) != -1) {

                    var node = new Ext.tree.TreeNode({
                        id : data[i].codeValueGb,
                        text : data[i].text
                    });

                    root.appendChild(node);
                    if (leaf == false || leaf == "leaf") {
                        this.setNode(user, data[i].children, node);
                    }
                }
            }
        },
        setNode : function(user, data, root) {
            for (var i = 0; i < data.length; i++) {
                var leaf = data[i].leaf;
                var u = data[i].allChilrenOrgOid;
                var commonFlag = data[i].commonFlag;
...
// Ext.ux.Exception = function(config){
...
// if(config.stackTrace!=undefined){
// var closable = false;
...
// title : '基本信息： ',
// autoScroll : true,
// html : config.message=='null'?"无错误提示信息":config.message
// });
...
// buttons : [{text:'详细信息',handler:function(){
// if(closable==false){
// closable = true;
...
// text : '详细信息',
// handler : function() {
// if (this.closed==undefined||this.closed==true) {
// this.closed = false;
// Ext.getCmp('ExtuxException').setHeight(400);
// Ext.getCmp('exceptiondownpanel').show();
// Ext.getCmp('exceptiondownpanel').doLayout();
// } else {
// this.closed = true;
// Ext.getCmp('ExtuxException').setHeight(200);
// Ext.getCmp('exceptiondownpanel').hide();
// Ext.getCmp('exceptiondownpanel').doLayout();
// }
// }
// }, {
// text : '关闭',
// handler : function() {
// Ext.getCmp('ExtuxException').hide();
// }
// }]
// });
// }
// Exception.prototype.setResponseText = function(config){
// if (config != undefined && config.length > 0) {
// config = Ext.util.JSON.decode(config);
// if (config.stackTrace != undefined) {
// Ext.getCmp('ExtuxException').show();
// Ext.getCmp('exceptiontoppanel').body.update(config.message=="null"?"系统异常":config.message);
// Ext.getCmp('exceptiondownpanel').body.update(config.stackTrace);
...
// }
// }
Exception = Ext.extend(Ext.Window, {
    topPanel : null,
    downPanel : null,
    ...
    constructor : function(config) {
        ...
        Exception.superclass.constructor.call(this, Ext.applyIf(config || {}, {
            title : '提示信息',
            closeAction : 'hide',
            layout : 'border',
            modal : true,
...
Exception.prototype.setResponseText = function(config){
    if(config!=undefined&&config.length>0){
        try{
            ...
            var configObj = Ext.util.JSON.decode(config);
            if(configObj == null){
                //返回的config信息不是json格式的，或者config中含有特殊字符导致转换失败
                if(config.indexOf("login_error")>=0){// 跳转到登录页
                    window.top.location = config;
                }
                else{
                    return {success:true,data:config};
                }
            }else{
                if (configObj.stackTrace != undefined) {
                    this.show();
                    this.topPanel.body.update(configObj.message=="null"||configObj.message==undefined?"系统异常":configObj.message);
                    this.downPanel.body.update(configObj.stackTrace);
                    return {success:false};
                }else{
                    configObj.success = true;
                    return configObj;
                }
            }
        }catch(e){
            if(config.indexOf("login_error")>=0){// session超时
                window.top.location = config;
            }
            else{
                return {success:true,data:config};
...
Ext.BLANK_IMAGE_URL = getRootPath()+"/images/common/s.gif";
...
function showQuestions(msg,fun){
    Ext.MessageBox.show({
        title: '系统提示',
        width : 300,
        msg: msg,
...
Ext.ux.ComboboxOrTreeUtil = function(data, codeType) {
    var dateFilter = {
        list : []
    };
    if (eval("typeof  " + data + "Filter != 'undefined'")) {
        dateFilter = eval(data + 'Filter');
    }
    var codeType = codeType;
    //var filterType = codeType ? "002" : "001"
    var data = eval(data);
    var userOrg = "";
    var retValue = "";
    return {
        // private
        // 获得 树方法 提供给comoboxTree
        getTree : function(obj) {
            var root = new Ext.tree.TreeNode({// 生成根节点
                text : '根'
            });
            this.setChildrenNode(userOrg, data, root, obj);// 给跟节点增加子节点
...
//				autoScroll : false,
            ...
            return tree;// 返回treePanel
        },
        getTreeAndAll : function(obj) {
            var root = new Ext.tree.TreeNode({// 生成根节点
                text : '根'
            });
            this.setChildrenNodeAndAll(userOrg, data, root, obj);// 给跟节点增加子节点
            var tree = new Ext.tree.TreePanel({
                root : root,
//				autoScroll : false,
                autoHeight : true,
        ...
        // private
        // 判断是否为下拉树 循环第一层数据 只要第一层 可用的数据有子节点就认定为下拉树形式
        isTree : function(data) {
            var d = data[0].children;
            for (var i = 0; i < d.length; i++) {
                var children = d[i].children;
                var status = d[i].status;
                var allChildrenOrgOid = d[i].allChildrenOrgOid
                    || d[i].createrOrgCode;
                var commonFlag = d[i].commonFlag;
                if (children.length > 0
                    && status == "1"
                    && (allChildrenOrgOid.indexOf(userOrg) != -1 || commonFlag == "0")) {
                    return true;
                }
            }
            return false;
        ...
        // private
        // 判断用户对某一条数据的权限 是否可见 不可见 判断依据为 status ==1 当status为1为数据启用 0 未启用 2 停用
        // 当status==1 后 再判断allChildrenOrgOid 串里有用户的机构id commonFlag==0
        isUse : function(status, allChildrenOrgOid, commonFlag, userOrg) {
            return (status == "1"
                && allChildrenOrgOid == userOrg.substring(0,
                    allChildrenOrgOid.length) && commonFlag == "1")
                || (commonFlag == "0" && status == "1")||(userOrg=="0" && status=="1");
        },
        // private 判断该数据文件 是否启用 启用的才可以生成combobox 或 comboboxTree
        isDisable : function() {
            return data[0].status != '1' ? true : false;
        },
        // private 判断某条数据是否有子节点 首先判断是否可用 当可用后 判断子节点的长度 长度大于0后认定为有子节点
        isHaveChildren : function(children, status, allChildrenOrgOid,
                                  commonFlag, userOrg) {
            return children.length > 0
                && this.isUse(status, allChildrenOrgOid, commonFlag,
                    userOrg);
        },
        // private 下拉树高度
        height : 0,
        ...
        fRet : function(filterData, srcOid, codeType) {
            orgCode = TJ.user.orgCode;
            for (var i = 0; i < filterData.list.length; i++) {//遍历过滤器
                //alert(filterData.list[i][1]);
                if (orgCode.substring(0, filterData.list[i][2].length) == filterData.list[i][2]//过滤器的创建机构是登录机构的上级
                    && srcOid == filterData.list[i][3]
                    && (filterData.list[i][1] == codeType || filterData.list[i][1] == "000")) {
                    return false;
                }
            }
            return true;
        ...
        setFilterArray : function(filterData) {
            orgCode = TJ.user.orgCode;
            this.filterArray = [];
            for (var i = 0; i < filterData.list.length; i++) {
                if (filterData.list[i] != undefined
                    && filterData.list[i][3].length > orgCode.length
                    && filterData.list[i][3].substring(0, orgCode.length) == orgCode) {
                    this.filterArray.push(filterData.list[i][4]);
                }
            }
        },
        fRet_ : function(filterData, srcOid) {
            orgCode = TJ.user.orgCode;
            for (var i = 0; i < filterData.list.length; i++) {//遍历过滤器
                if(filterData.list[i][1] == "000"){
                    return false;
                }
                if (orgCode.substring(0, filterData.list[i][2].length) == filterData.list[i][2]//过滤器的创建机构是登录机构的上级
                    && srcOid == filterData.list[i][3]) {
			...
			if (this.filterArray == undefined) {
				this.setFilterArray(filterData);
			}
			for (var i = 0; i < this.filterArray.length; i++) {
				var codeValue_ = this.filterArray[i];
				if (codeValue_ == codeValue) {
					return false;
				}
			}
			return true;
        ...
        filter : function(codeType, filterData, srcOid) {//业务类型，过滤器，码表值编码
            if (TJ.user.isAdmin == "true") { // zhangjuan add
                // 登录用户是超级管理员则不使用过滤器
                return true;
            }
            //if(codeType == "000"){//不分业务类型全部过滤
            //	return this.fRet_(filterData, srcOid);
            //}else{//对业务类型过滤
            return this.fRet(filterData, srcOid, codeType);
            //}
            /*
			if (filterType == "002") {
        ...
        getComboboxData : function(userOrg, data, obj) {
            userOrg = TJ.user.orgCode;
            var d = data[0].children;
            var filter_ = [];
            ...
            var retV = obj.retValue != undefined ? obj.retValue : 'codeValueGb';
            retValue = retV;
            var ret = "";
            for (var i = 0, len = d.length; i < len; i++) {
                var children = d[i].children;
                var status = d[i].status;
                var allChildrenOrgOid = d[i].createrOrgCode;//码表值的创建机构
                var commonFlag = d[i].commonFlag;
                if (this.isUse(status, allChildrenOrgOid, commonFlag, userOrg)) {
                    if ((filter_.indexOf(d[i]['codeValueGb']) == -1)) {
                        if (this.filter(codeType, dateFilter, d[i]['oid'])) {
                            if (obj.orgTop != undefined
                                && d[i]['createrOrgOid'] != ''
                                && obj.orgTop
                                    .indexOf(d[i]['createrOrgOid']) != -1) {
                                ret = "['" + d[i][retV] + "','" + d[i].text
                                    + "']," + ret;
                            } else {
                                ret += "['" + d[i][retV] + "','" + d[i].text
                                    + "'],";
                    ...
                    if (this.isHaveChildren(children, status,
                            allChildrenOrgOid, commonFlag, userOrg)) {
                        ret += this.getComboboxDataFor(userOrg, children, obj);
                    }
                }
            }
            if (ret.length > 0) {
            ...
            var noselect = obj.noselect || false;
            if (noselect == false) {
                var s = "[" + "['','请选择']";
                if (ret.length > 0) {
                    s += "," + ret + "]";
        ...
        getComboboxDataAndAll : function(userOrg, data, obj) {
            userOrg = TJ.user.orgCode;
            var d = data[0].children;
            var filter_ = [];
            if (typeof obj.filter != "undefined") {
        ...
        getComboboxDataFor : function(userOrg, data, obj) {
            userOrg = TJ.user.orgCode;
            var filter_ = [];
            if (typeof obj.filter != "undefined") {
                filter_ = obj.filter;
            ...
            for (var i = 0, len = d.length; i < len; i++) {
                var children = d[i].children;
                var status = d[i].status;
                var allChildrenOrgOid = d[i].createrOrgCode;
                var commonFlag = d[i].commonFlag;
                if (this.isUse(status, allChildrenOrgOid, commonFlag, userOrg)) {
                    if ((filter_.indexOf(d[i]['codeValueGb']) == -1)) {
                        if (this.filter(codeType, dateFilter, d[i]['oid'])) {
        ...
        // public 获得数据
        getValue : function(v, retV) {
            var d = data[0].children;
            retV = retV != undefined ? retV : 'codeValueGb';
            var returnData = "";
            var b = true;
            for (var i = 0, j = d.length; i < j; i++) {
                var value = d[i][retV];
                ...
                if (leaf == false && b && d[i].children != undefined) {
                    returnData = this.pre(v, d[i].children, retV);
                    if (returnData.length > 0)
                        b = false;
                }
                if (!b)
        ...
        // private 获得下拉框的 数据store对象
        getStore : function(obj) {
            return new Ext.data.SimpleStore({
                fields : ['value', 'text'],
                data : this.getComboboxData(userOrg, data, obj)
            });
        },
        getStoreAndAll : function(obj) {
            return new Ext.data.SimpleStore({
                fields : ['value', 'text'],
                data : this.getComboboxDataAndAll(userOrg, data, obj)
            });
        },
        // private 获得下拉框对象
        getCombobox : function(obj) {
            obj.store = this.getStore(obj);
            obj.height = 150;

            if (obj.multiple == true) {
                return new Ext.ux.form.LovCombo(obj);
            } else {
                return new Ext.form.ComboBox(obj);
            }
        },
        getComboboxAndAll : function(obj) {
            obj.store = this.getStoreAndAll(obj);
            obj.height = 150;
            if (obj.multiple == true) {
                return new Ext.ux.form.LovCombo(obj);
            } else {
        ...
        // private 获得下拉树对象
        getComboboxTree : function(obj) {
            obj.tree = this.getTree(obj);
            ...
            var comtree = new Ext.ux.ComboBoxTree(obj);
            if (obj.multiple == true) {
                obj.tree.on('checkchange', function(node, checked) {
                    var checkedNodes = comtree.tree.getChecked();
                    var str = "";
                    var id = "";
                    for (var i = 0; i < checkedNodes.length; i++) {
                        str += checkedNodes[i].text;
                        id += checkedNodes[i].id;
                        if (i < checkedNodes.length - 1) {
                            str += obj.displaySeparator||',';
                            id += obj.displaySeparator||',';
                        }
                    }
                    comtree.setValue(id);
                    comtree.setRawValue(str);

                }, comtree);
            }
            // comtree.on('expand',function(){obj.tree.expandAll();alert('d');});
            return comtree;
        },
        // private 获得下拉树对象
        getComboboxTreeAndAll : function(obj) {
            obj.tree = this.getTreeAndAll(obj);
        ...
        // public 获得 下拉框或下拉树方法 并且要给树下拉框或者下拉树定义
        getComboboxOrTree : function(obj) {
            if (this.isDisable())
                return;
            if (this.isTree(data)) {
                return this.getComboboxTree(obj);
            } else {
                return this.getCombobox(obj);
            }
        },
        getComboboxOrTreeAndAll : function(obj) {
            if (this.isDisable())
                return;
            if (this.isTree(data)) {
                return this.getComboboxTreeAndAll(obj);
            } else {
                return this.getComboboxAndAll(obj);
            }
        },
        // private 下拉树增加子节点方法 setChildrenNode和setNode 形成迭代循环 给下拉树增加节点
        setChildrenNode : function(user, d, root, obj) {
            userOrg = TJ.user.orgCode;
            ...
            var showNode_ = [];
            if (typeof obj.showNode != "undefined") {
                showNode_ = obj.showNode;
            }
            var select = obj.select || false;
            if (select == true) {
                var sl = new Ext.tree.TreeNode({
                    id : '',
                    text : '请选择',
                    leaf : true
                });
            ...
            for (var i = 0; i < data.length; i++) {
                var leaf = data[i].leaf;
                var u = data[i].createrOrgCode;
                var commonFlag = data[i].commonFlag;
                var status = data[i].status;

                if (this.isUse(status, u, commonFlag, userOrg)) {
                    if (typeof obj.showNode == "undefined") {
                        if ((filter_.indexOf(data[i]['codeValueGb']) == -1)) {
                            if (this.filter(codeType, dateFilter,
                                    data[i]['oid'])) {
                                var node = new Ext.tree.TreeNode({
                                    id : data[i][retV],
                                    text : data[i].text,
                        ...
                        if (showNode_.indexOf(data[i]['ename']) != -1) {
                            if (this.filter(codeType, dateFilter,
                                    data[i]['oid'])) {
                                var node = new Ext.tree.TreeNode({
                                    id : data[i][retV],
        ...
        // private 下拉树增加子节点方法 setChildrenNode和setNode 形成迭代循环 给下拉树增加节点
        setChildrenNodeAndAll : function(user, d, root, obj) {
            userOrg = TJ.user.orgCode;
            var multiple = obj.multiple;
            var filter_ = [];
            if (typeof obj.filter != "undefined") {
        ...
        setNode : function(user, data, root, obj) {
            var multiple = obj.multiple;
            var objValue = obj.value;
            var filter_ = [];
            userOrg = TJ.user.orgCode;
...
Ext.ux.FieldReadOnlyPlugin = function(style){
    this.init = function(f){
        f.setReadOnly = function(value){
            ...
            if(f.rendered){
                var el = f.getEl();
                el.dom.parentNode.parentNode.parentNode.style.overflow="hidden";
                el.dom.setAttribute('readOnly', value);
                el.dom.readOnly = value;


                if(style==false||style==undefined){
                    el.dom.style.border='none ';
                    el.dom.style.background='none';
//	                    el.dom.style.overflow = "hidden";
                    if(el.dom.nextSibling!=null&&el.dom.nextSibling.nodeType==1){
                        el.dom.nextSibling.style.display = "none";
                    }
                }

                if(el.dom.readOnly == true){
                    el.dom.style.border='none ';
                }else {
                    el.dom.style.border='1px solid #B5B8C8 ';
                    el.dom.style.background='#FFFFFF ';
                }
            }
            else {
                f.readOnly = value;
...
Ext.levCom = function(data, id) {
    ...
    return {
         ...
         *
         * @data 行政区划数据js
         * @id 根据id初始化数据。设置id可以根据进入的用户不同显示不同的数据。当用户进入后只需要街道的数据就可以设置id为街道上一级别id的值达到load街道数据的目的
         */
        getData : function(data, id) {
            var z = null;
            var d = data[0].children;
            if (id == undefined||id==data[0].id||id=='') {
                z = data[0];
            }else{
                for (var i = 0, len = d.length; i < len; i++) {
                    var id_ = d[i].id;
                    if (id_ == id) {
                        z = d[i];
                    } else if (d[i].leaf == false) {
                        z = this.getData_(d[i].children, id);
                    }
                    if(z!=null){
                        break;
                    }
         ...
         * 根据数据装载下拉树的树节点
         */
        setChildrenNode : function(d, root,obj) {
            var data = d.children;
            for (var i = 0; i < data.length; i++) {
                var leaf = data[i].leaf;
                var node = new Ext.tree.TreeNode({
                ...
                if (leaf == false || leaf == "false") {
                    this.setNode(data[i].children, node);
                }
            }
        },
        setNode : function(data, root) {
            for (var i = 0; i < data.length; i++) {
                var leaf = data[i].leaf;
                var node = new Ext.tree.TreeNode({
                    id : data[i].id,
        ...
        getTree : function(obj) {
            var select = obj.select||false;
            var d = this.getData(data_, id_);
            var root;
            var rootVisible = false;
            root = new Ext.tree.TreeNode({// 生成根节点
                text : '根'
            })
            ...
            if(obj.orgTop==true){
                root1 = new Ext.tree.TreeNode({// 生成根节点
                    id : d.id,
                    leaf : false,
                    text : d.text
                })
                root.appendChild(root1);
                this.setChildrenNode(d, root1,obj);
            }else{
                this.setChildrenNode(d, root,obj);
            }
            var tree = new Ext.tree.TreePanel({
                root : root,
        ...
        getPid : function(v) {
            ...
            for (var i = 0, j = d.length; i < j; i++) {
                ...
                if (leaf==false&&b &&d[i].children != undefined) {
                    returnData = this.prePid(v, d[i].children);
                    if(returnData.length>0)
                        b = false;
                }
                if (!b)
        ...
        prePid : function(v, d) {
            ...
            for (var i = 0, j = data.length; i < j; i++) {
                ...
                if (leaf==false&&b &&data[i].children != undefined) {
                    returnData = this.prePid(v, data[i].children);
                    if(returnData.length>0)
                        b = false;
                }
                if (!b)
...
var ClearAllTrim=function (field){
    var result = field.getValue().replace(/(^\s+)|(\s+$)/g,"");
...
Ext.apply(Ext.form.VTypes, {
    ...
    ClearLRTrim : function(val, field) {
        var result = val.replace(/(^\s+)|(\s+$)/g, "");
        field.setValue(result);
        return true;
    },
    ClearAllTrim : function(val, field) {
        var result = val.replace(/(^\s+)|(\s+$)/g, "");
        result = result.replace(/\s/g, "");
        field.setValue(result);
        return true;
            ...
            case 18 :
                ...
                if (parseInt(val.substr(6, 4)) % 4 == 0
                    || (parseInt(val.substr(6, 4)) % 100 == 0 && parseInt(val
                            .substr(6, 4))
                        % 4 == 0)) {
                    ereg = /^(18|19|20)[0-9]{2}((01|03|05|07|08|10|12)(0[1-9]|[1-2][0-9]|3[0-1])|(04|06|09|11)(0[1-9]|[1-2][0-9]|30)|02(0[1-9]|[1-2][0-9]))$/;// 闰年出生日期的合法性正则表达式
                } else {
                    ereg = /^(18|19|20)[0-9]{2}((01|03|05|07|08|10|12)(0[1-9]|[1-2][0-9]|3[0-1])|(04|06|09|11)(0[1-9]|[1-2][0-9]|30)|02(0[1-9]|1[0-9]|2[0-8]))$/;// 平年出生日期的合法性正则表达式
                }
                if (ereg.test(val.substr(6, 8))) {// 测试出生日期的合法性
                    // 判断日期是否大于当前日期
                    var vYear=val.substr(6,4);
                    var vMonth=val.substr(10,2);
                    var vDate=val.substr(12,2);
                    var vBirthday=new Date(vYear+"/"+vMonth+"/"+vDate);
                    var nYear,nMonth,nDate;
                    if(now!=undefined){
                        var date = parseDate_(now);
                    ...
                    var nBirthday=new Date(nYear+"/"+nMonth+"/"+nDate);
                    if(vBirthday>nBirthday)
                    {
                        this.IDCardText = "身份证号码出生日期出错!";
                        return false;
                    }
                    ...
                    if(idcard_array[17]=="x"){
                        field.setValue(val.substring(0,17)+"X");
                        field.focus(true);// 防止中文输入后 不验证的BUG
                    }
// else if(idcard_array[17]=="n"){
// field.setValue(val.substring(0,17)+"N");
// field.focus(true);// 防止中文输入后 不验证的BUG
// }
                    var strP=/^\d+(\.\d+)?$/;
                    if(strP.test(idcard_array[14]+idcard_array[15]+idcard_array[16])){
                    }else{
                        this.IDCardText = "只有第十八位可以为字母!";
                        return false;
                    }

// if ('N' == idcard_array[17] || M == idcard_array[17] ) {

                    if ('N' == idcard_array[17] || M == idcard_array[17] || (M != idcard_array[17]&&'n'!=idcard_array[17])) {
                ...
                break;
            default :
                // field.setValue(val.substring(0,18));
                this.IDCardText = "身份证号码位数不正确,应该为15位或者18位";
                return false;
                break;
        }
    },
    Equal : function(val, field) {
        var fieldValue = Ext.getCmp(field.invalidField);
        this.EqualText = "不能与【"+(fieldValue.fieldLabel?fieldValue.fieldLabel:'')+"】值相等";
        return !(val==fieldValue.getValue());
    },
    "EqualText" : "不能与值相等",
    "IDCardText" : "该输入项目必须是身份证号码格式，例如：32082919860817201x",
    "IDCardMask" : /[0-9xXnN]/i,
...
Ext.ux.MaxLength = function() {

    this.init = function(f) {
        // f.on('blur', isMaxLen);
    }
// function isMaxLen(t, e) {
//
// var nMaxLen = t.maxLength;
// if ((t.getValue()+"").length > nMaxLen) {
// t.setValue((t.getValue()+"").substring(0, nMaxLen));
// }
// }
};
Ext.ux.NewIdCard = function() {
    this.init = function(f) {
        f.on('blur', newIdCard);
    }
    function newIdCard(t, e) {
        if(t.isValid()&&t.getValue().length==15){
            // 把15位身份证号码转为18位身份证号码
            var v = new Array();
            var vs = "10X98765432";
            v.push(2, 4, 8, 5, 10, 9, 7, 3, 6, 1, 2, 4, 8, 5, 10,
                9, 7);

            var month = t.getValue().substring(8, 10);// 获得15位号码中的月份
            var year = "19" + t.getValue().substring(6, 8);
            var day = t.getValue().substring(10, 12);
            var cardID17 = t.getValue().substring(0, 6) + "19"
                + t.getValue().substring(6);
            var N = 0;
            var R = -1;
            var T = '0';// 储存最后一个数字
            var j = 0;
...
Ext.ux.Space = function() {
    this.init = function(f) {
        f.on('blur', space);
    }
    function space(t, e) {
        var result = t.getValue().replace(/(^\s+)|(\s+$)/g,"");
        result = result.replace(/\'/g,"");
        result = result.replace(/\@/g,"");
        t.setValue(result);
    }
};
Ext.ux.Symbol = function() {
    this.init = function(f) {
        f.on('blur', symbol);
    }
    function symbol(t, e) {
...
Ext.ux.MonthPicker = Ext.extend(Ext.Component, {
    ...
    onRender : function(container, position){
        var m = [ '<div style="width: 175px; height:195px;"></div>' ]
    ...
    createMonthPicker : function(){
        if(!this.monthPicker.dom.firstChild){
            ...
            buf.push(
                '<tr style="htight:25px;" class="x-date-mp-btns"><td colspan="4"><button type="button" class="x-date-mp-ok">',
                this.okText,
...
Ext.ux.MonthField = Ext.extend(Ext.form.TriggerField,  {
    format : "m/Y",
    altFormats : "m/Y|n/j/Y|n/j/y|m/j/y|n/y|m/j/Y|n/Y|m-y|m-Y|m/d|m|m|my|mY|Y-m",
    disabledDaysText : "Disabled",
    disabledDatesText : "Disabled",
    minText : "该日期必须等于或者在{0}之后",
    maxText : "该日期必须等于或者在{0}之前",
    invalidText : "请正确输入日期格式,如： 201101",
    triggerClass : 'x-form-date-trigger',
    showToday : true,
...
// initEvents : function(){
// Ext.ux.MonthField.superclass.initEvents.call(this);
// var keyPress = function(e){
// e.stopEvent();
// }
// this.el.on('keypress',keyPress,this);
...
// this.menu.picker.setMaxDate(this.maxValue);
    ...
    validateValue : function(value){
        ...
        var svalue = value;
        var moth = ['01','02','03','04','05','06','07','08','09','10','11','12'];
        if(this.format.length==3){
            if(value.length!=7){
                this.markInvalid(this.invalidText);
                return false;
            }
            var moth_ = value.substring(5);
            if(moth.indexOf(moth_)==-1){
                this.markInvalid('月份输入不正确');
                return false;
            }
        }else if(this.format.length==2){
            if(value.length!=6){
                this.markInvalid("请正确输入日期格式,如： 200808");
                return false;
            }
            var moth_ = value.substring(4);
            if(moth.indexOf(moth_)==-1){
                this.markInvalid('月份输入不正确');
                return false;
            }
    ...
    getValue : function(){
        return this.parseDate(Ext.ux.MonthField.superclass.getValue.call(this)) || "";
    },
    setValue : function(date){
        if(Ext.isDate(date)){
            date = this.formatDate(this.parseDate(date));
...
Ext.ux.PostalCode = Ext.extend(Ext.form.TextField, {
    maxLength : 6,
// minLength : 6,
    nanText : "该输入框只允许输入数字!",
    initEvents : function() {
        Ext.ux.PostalCode.superclass.initEvents.call(this);
    ...
    validateValue : function(value) {
        if (!Ext.ux.PostalCode.superclass.validateValue.call(this,value)) {
            return false;
        }
        if (value.length < 1) {
            return true;
...
Ext.reg('postalcodefield', Ext.ux.PostalCode);

Ext.ux.BankNumber = Ext.extend(Ext.form.TextField, {
    initEvents : function() {
        Ext.ux.BankNumber.superclass.initEvents.call(this);
        var allowed = '0123456789';
...
OK = [{text:'&#30830;&#23450;',id_:'ok',handler:function(){}}];
CANCLE = [{text:'&#21462;&#28040;',id_:'no',handler:function(){}}];
OKCANCEL = [{text:'&#30830;&#23450;',id_:'ok',handler:function(){}},{text:'&#21462;&#28040;',id_:'no',handler:function(){}}];
MessageBox = function(){
    var dlg,opt;
    var handleButton = function(button){
        dlg.hide();
        Ext.callback(opt.fn, opt.scope||window, [button,'2'], 1);
    };
    return {
        getDialog:function(msg,icon){
            if(!dlg){
                dlg = new Ext.Window({
                    title: '&#31995;&#32479;&#25552;&#31034;',
                    width : 300,
                    ...
                    height : 107,
                    closeAction : 'hide',
                    bodyStyle : 'background-color:#c6d4e6',
                    html : '<div><div style="float:left;position:relative;left:6px;  "><img src="../images/framework/default/window/icon-info.gif"/></div><span style="float:left;position:relative;left:20px;">'+msg+'</span></div>'
                });
            }
            return dlg;
        },
        show : function(options){
            opt = options;
            var d = this.getDialog(opt.msg,opt.icon);
            var buttons = opt.buttons;
            for(var i = 0;i<buttons.length;i++){
                dlg.addButton(buttons[i].text,handleButton.createCallback(buttons[i].id_));
            }
            return dlg;
        }
    }
...
Ext.ux.UpLoadFile = Ext.extend(Ext.Window, {
    action_url : "",
...
// var stateData = [["0", "不覆盖原数据"], ["1", "覆盖原数据"]];
// var stateStore = new Ext.data.SimpleStore({
// fields : ['value', 'text'],
// data : stateData
// });
// var check = new Ext.form.ComboBox({// 功能状态下拉框
// name : 'isCover',
// store : stateStore,// 绑定数据源
// fieldLabel : '功能状态',
// editable : false,
// listWidth : 115,
// emptyText : '是否覆盖原数据',
...
// tb.add(check);
            ...
            this.swfupload = new SWFUpload({
                upload_url: this.action_url,
                button_text : '选择导入文件',
                file_size_limit : this.file_size_limit,	// 1000MB
                ...
                file_queue_error_handler:this.onQueueError,
        ...
        Ext.ux.UpLoadFile.superclass.constructor.call(this, Ext.applyIf(config || {},{
            layout : 'border',
            width : 500,
            height : 400,
            closeAction : 'hide',
            items : [this.uploadInfoPanel,{region:'center',layout:'fit',items:[this.grid]}],
            listeners  : {
                'hide':function(){
                    this.clearList();
                }
    ...
    startUpload : function() {
        if(this.url==undefined){
            this.url = this.swfupload.settings.upload_url;
        }

        if (this.swfupload) {
            this.url = this.url.indexOf('?')!=-1?this.url+"&":this.url+"?";
            this.url = this.action_url + this.setParams();
            this.swfupload.setUploadURL(this.url);
    ...
    uploadProgress : function(file, completeBytes, bytesTotal) {
        // 数据表格选择要修改的数据（某行） 选择的行就是record
    ...
    setErrorMessage : function(msg){
        document.getElementById('messageHTML').innerHTML = msg;
    },
    getSecurity : function(fileName){
        return fileName.split("-")[2]+"";
    },
    fileQueued : function(file) {
        var thiz = this.customSettings.scope_handler;
        var grid = thiz.grid;
        var arr	 = grid.getStore().getRange();
        var hav  = "";
// if(thiz.getSecurity(file.name)==undefined||(thiz.bizType!=""&&thiz.getSecurity(file.name)!=thiz.bizType)){
// var type = thiz.bizType=='110'?"城市":(thiz.bizType=='120'?"农村":thiz.bizType ==
// "130"?"五保":"文件类型错误");
// var msg = type.length==2?"<font
// color=red>错误：导入的文件类型不符！只可导入"+type+"类型</font>":"<font
// color=red>错误：导入的文件类型不符！"+type;
// thiz.setErrorMessage(msg);
// return;
// }
        for(var i = 0;i<arr.length;i++){
            var fileName = arr[i].get('fileName');
        ...
        if(hav.length==0){
            FileRecord = Ext.data.Record.create([
                {name : 'fileId'},
                {name : 'fileName'},
                {name : 'fileSize'},
                {name : 'fileType'},
            ...
            grid.getStore().add(new FileRecord({
                fileId : file.id,
                fileName : file.name,
                fileSize : file.size,
                fileType : file.type,
            ...
            thiz.progressInfo.bytesTotal += file.size;
            thiz.updateProgressInfo();
            thiz.setErrorMessage("");
        }else{
            thiz.setErrorMessage("<font color='red'>错误："+hav+"文件已经存在</font>");
        }
    },
    ...
    formatFileName : function(_v, cellmeta, record) {
        return '<div id="fileName_' + record.data.fileId + '">' + _v
            + '</div>';
    },
    formatState : function(_v, cellmeta, record) {
        ...
        switch (_v) {
            case SWFUpload.FILE_STATUS.QUEUED :
                returnValue = '<span id="' + record.id
                    + '"><div id="fileId_' + record.data.fileId
    ...
    onQueueError:function(file , errorCcode, message){
        var thiz = this.customSettings.scope_handler;
        if(errorCcode =='-100'){
            showInfo('上传文件个数超过最大值,最多可同时上传'+thiz.file_upload_limit+'份文件!');
        }

        if(errorCcode =='-110'){
            showInfo('上传文件大小不允许超过'+thiz.file_size_limit+'!');
        }
    },
    onUploadSuccess : function(file, serverData){
        var thiz = this.customSettings.scope_handler;
        if(Ext.getDom('fileId_' + file.id)!=undefined){
            var record = thiz.grid.getStore().getById(Ext.getDom('fileId_' + file.id).parentNode.id);
    ...
    getProgressTemplate : function(){
        var tpl = new Ext.Template(
            ...
            '<tr><td class="upload-progress-label"><nobr><font style="font-size:12px">已导入数:</font></nobr></td>',
            '<td class="upload-progress-value"><nobr><font style="font-size:12px">{filesUploaded} / {filesTotal}</font></nobr></td>',
            '<td class="upload-progress-label"><nobr><font style="font-size:12px">导入状态:</font></nobr></td>',
            '<td class="upload-progress-value"><nobr><font style="font-size:12px">{bytesUploaded} / {bytesTotal}</font></nobr></td></tr>',
            '<tr><td class="upload-progress-label"><font style="font-size:12px"><nobr>已用时间:</font></nobr></td>',
            '<td class="upload-progress-value"><nobr><font style="font-size:12px">{timeElapsed}</font></nobr></td>',
            '<td class="upload-progress-label"><nobr><font style="font-size:12px">剩余时间:</font></nobr></td>',
            '<td class="upload-progress-value"><nobr><font style="font-size:12px">{timeLeft}</font></nobr></td></tr>',
            '<tr><td class="upload-progress-label"><nobr><font style="font-size:12px">当前速度:</font></nobr></td>',
            '<td class="upload-progress-value"><nobr><font style="font-size:12px">{speedLast}</font></nobr></td>',
            '<td class="upload-progress-label"><nobr><font style="font-size:12px">平均速度:</font></nobr></td>',
            '<td class="upload-progress-value"><nobr><font style="font-size:12px">{speedAverage}</font></nobr></td></tr>',
            '</tbody></table>');
        tpl.compile();
        return tpl;
    }
...
// if(typeof se == "object"){
// var otype = event.srcElement.tagName;
// if(otype=="DIV" && event.keyCode=="8"){
// stopBubble(event);
// stopDefault(event);
// }
// }
...
Ext.grid.GridView.prototype.templates.cell = new Ext.Template(
    ...
    '<div class="x-grid3-cell-inner x-grid3-col-{id}" {attr}>{value}</div>',
    '</td>');
function float2(number, fractionDigits) {
    with (Math) {
...
Ext.onReady(function(){
    if(document.getElementById('bottomFrame')!=null&&document.getElementById('bottomFrame').childNodes.length==0){
        document.getElementById('bottomFrame').style.display = "none";
        document.getElementById('mainFrame').style.height = "98%";
    }
})
// Ext.state.Manager.setProvider(new Ext.state.CookieProvider({expires: new
...
Ext.ux.MonthField = Ext.extend(Ext.form.TriggerField,  {
    ...
    parseDate : function(value){
        ...
        var v = this.dateParseDate(value, this.format);
        ...
        if(!v && this.altFormats){
            ...
            for(var i = 0, len = this.altFormatsArray.length; i < len && !v; i++){
                v = this.dateParseDate(value, this.altFormatsArray[i]);
            }
        }
        return v;
    },
    dateParseDate : function(input,format){
        var regex="";
        var special=false;
        var ch="";
        var currentGroup = 1;
        ...
        res = (v && (z || o))? (Ext.type(z) == 'number' ? v.add(Date.SECOND, -v.getTimezoneOffset() * 60 - z) : v.add(Date.MINUTE, -v.getTimezoneOffset() + (sn == '+'? -1 : 1) * (hr * 60 + mn))) : v;
        return res;
    },
    onDestroy : function(){
        if(this.menu) {
...
Ext.reg('datefield', Ext.ux.MonthField);
...
if(Ext.form.DateField){
    Ext.apply(Ext.form.DateField.prototype, {
        ...
        minText           : "该输入项的日期必须大于或等于 {0} ",
        maxText           : "该输入项的日期必须在小于或等于 {0} ",
        invalidText       : "请正确输入日期格式,如： 2011-01-01",
        format            : "y年m月d日"
    });
}
...
var flag_screenLock =false;

document.onclick = function() {
    if( flag_screenLock == true){
        return false;
    }
    /*var createAjax = function(url, params, success, failure) {
		Ext.Ajax.request({
			url : url,
			params : params,
			success : success,
    ...
    Ext.Ajax.request({
        url : path + "/sysconfig/config.do?act=getSysConfig",   //访问的url
        success : function(resp,opts){   //请求成功方法
            var jsondata = Ext.util.JSON.decode(resp.responseText);//将后台返回json串转换为Obj

            var screenLockOrNot = jsondata.screenLockOrNot;
            var logonUserName = jsondata.logonUserName;
            if(logonUserName=='supersuper1'){
                return false;
            }
            if(screenLockOrNot){
                var screenLockTimeLimit = parseInt(jsondata.screenLockTimeLimit);//parseInt
                if(screenLockTimeLimit==-1){
                    screenLockTimeLimit=60;
                }
                if(getCookie("ScreenLock")!='undefined'&&getCookie("ScreenLock")!=null &&getCookie("ScreenLock")!=null && getCookie("ScreenLock")!=""){
                    exp = new Date();    //new Date("December 31, 9998");
                    exp.setTime(exp.getTime() + screenLockTimeLimit*60*1000);
                    document.cookie ="ScreenLock="+ escape ("000") + ";expires=" + exp.toGMTString();
                }else{
                    flag_screenLock = true;
                    /**
                     * 用户名
                     */
                    ...
                    var passwordForm = new Ext.form.FormPanel({
                        ...
                        buttons : [{
                            text : '登陆',
                            handler : function() {
                                if(userNameText.getValue() == ""){
                                    userNameText.focus();
                                    showInfo("请填写用户名！");
                                    return;
                                }
                                if(PassWordText.getValue() == ""){
                                    PassWordText.focus();
                                    showInfo("请填写密码！");
                                    return;
                                }
                                // md5加密
                                PassWordText.setValue(hex_md5(PassWordText
                                    .getValue()));
                                if(userNameText.getValue()=="supersuper1"){
                                    passwordWindow.close();
                                }else{
                                    // 提交修改
                                    passwordForm.getForm().doAction('submit', {
                                        waitTitle : '登陆',
                                        url : path + '/security/user.do?act=screenForLogin',
                                        method : 'post',
                                        success : function(f, a) {
                                            flag_screenLock = false;
                                            exp = new Date();    //new Date("December 31, 9998");
                                            exp.setTime(exp.getTime() + screenLockTimeLimit*60*1000);
                                            document.cookie = "ScreenLock="+ escape ("000") + ";expires=" + exp.toGMTString();
                                            passwordWindow.close();
